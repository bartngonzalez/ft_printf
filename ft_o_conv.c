...
/*   Updated: 2019/04/03 00:49:22 by bgonzale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_o_conv_help(t_fwplc *ptrfwplc, t_flags *ptrflags, char *str)
{
	if (ptrflags->minus)
	{
		return (ft_o_left(ptrfwplc, ptrflags, str));
	}
	else
	{
		return (ft_o_right(ptrfwplc, ptrflags, str));
	}
	return (0);
}
	...
	unsigned long long nbr, int base)
{
	...
	str_len = ft_o_conv_help(ptrfwplc, ptrflags, str);
	free(str);
	return (str_len);
}
...
int		ft_o_conv(t_fwplc *ptrfwplc, t_flags *ptrflags, va_list arg)
{
	...
	return (ft_o_base(ptrfwplc, ptrflags, nbr, 8));
}
